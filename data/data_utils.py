...
def balance(x, y):
    ...
    indices_max_class = rng.choice(
        np.where(indices[sorted_classes[-1]])[0], n_samples_min_class, replace=False
    )
...
def check_if_task_too_easy(
        ...
        preprocessor = ColumnTransformer(
            transformers=[
                ...
                (
                    ...
                    [i for i in range(X.shape[1]) if categorical_indicator[i]],
                ),
            ]
        )
    ...
    for iter in range(n_iters):
        ...
        if regression:
            score_hbgt = -mean_squared_error(
                y_test, hbgt.predict(X_test), squared=False
            )
        else:
...
def specify_categorical(X, dataset_id):
    ...
    for col in X.columns:
        if (
            ...
            or col.lower().endswith(".id")
            or col.lower().endswith("_index")
            or col.lower().endswith("-index")
            or col.lower().endswith(".index")
        ):
            res.append(col)
    if dataset_id == "42803":
